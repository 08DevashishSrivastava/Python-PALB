# 1.The code that prints the sum of the values from 1 to 3 using the print() function is print(1+2+3). With reference to this 
# code, print the sum of the values from 1 to 5 as follows.

solution = print(1 + 2 + 3 + 4 + 5)

# 2a.The figure below is a flowchart of a program that finds the sum of odd numbers among integers between 1 and 100 and 
...
solution = n = int(input("Enter a positive integer: "))
...
# 4.Write and save the following name and address in the name and address variables, respectively. Write a code that 
# prints them to the screen.

solution = name = "Devashish Srivastava"
address = "Kasna, Greater Noida, Uttar Pradesh, India"

print(name)
print(address)

# 5. Expect the output of the following code and write it down.
...
# 6.Write a program that takes two random integers as input and lists them from smallest to largest.
...
#  7. Write a program that performs the following using a compound conditional expression of an if statement.
# Coding guideline: Write a code that performs differently depending on whether the answer to the first question is 0 or 1, 
# as shown below.

solution = adult = int(input("Are you an adult? (1 if you are an adult, 0 if you are minor): "))
...
# 8.Among the positive natural numbers other than 1, a number that is not prime is called a composite number. Print the 
...
solution = for num in range(2, 13):
is_prime = True
...
# 9. An Armstrong number is a three-digit integer that is equal to the sum of the cubes of each digit. Find all Armstrong 
...
solution = print("Three-digit Armstrong numbers:", end=" ")
...
# 10.If there are two lists l1 and l2 with the following strings, use the combination of l1 and l2 to print as follows. Coding guideline: Use nested loops to print it out
solution = l1 = ['I like', 'I love']
...
# 11.The person dictionary is defined as follows. Add a new item to this person dictionary with the key 'Father' and the value 
...
# 12.With tuple data, the values of two variables can be swapped without using a temporary variable. Using this swapping 
...
solution = lst = [5, 6, 3, 9, 2, 12, 3, 8, 7]
...
# 13.The two-dimensional array a contains the values [[1, 2], [3, 4], [5, 6]]. Change this two-dimensional array to a onedimensional array like [1, 2, 3, 4, 5, 6], and print it out.
...
# 14.There is a dictionary variable maria as follows. In this dictionary variable, courses such as ‘korean' and ‘english' and their 
# scores are stored as key:value. Print the average score of 89.25 for maria's subject scores.
...
# that 'copy' to another variable, school2. (Check that school and school2 are different variables through the is operator.) 
# Coding guideline: Print so that the result of school is school2 is false.
